...
ClientPTZ::ClientPTZ(std::string url, std::string user, std::string password, bool showCapabilities) : ClientDevice(url, user, password, showCapabilities){
    if(_hasPTZ){
...
int ClientPTZ::getPTZConfigurations(){
...
int ClientPTZ::getStatus(std::string profileToken, StatusPTZ &status){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str()))
    {
        return -1;
    }
    ...
    tptz__GetStatus->ProfileToken = profileToken;
...
int ClientPTZ::absoluteMove(std::string profileToken, float pan, float panSpeed, float tilt, float tiltSpeed, float zoom, float zoomSpeed){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str()))
    {
        return -1;
    }
...
int ClientPTZ::relativeMove(std::string profileToken, float pan, float panSpeed, float tilt, float tiltSpeed, float zoom, float zoomSpeed){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str()))
    {
        return -1;
    }
...
int ClientPTZ::continuousMove(std::string profileToken, float panSpeed, float tiltSpeed, float zoomSpeed){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str()))
    {
        return -1;
    }
...
int ClientPTZ::stop(std::string profileToken, bool panTilt, bool zoom){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str()))
    {
        return -1;
    }
...
int ClientPTZ::setHomePosition(std::string profileToken){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str())){
        return -1;
    }
...
int ClientPTZ::goToHomePosition(std::string profileToken){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str())){
        return -1;
    }
...
int ClientPTZ::getConfiguration(std::string configurationToken){
    if (SOAP_OK != soap_wsse_add_UsernameTokenDigest(proxyPTZ.soap, NULL, _user.c_str(), _password.c_str())){
        return -1;
    }
    ...
    if(SOAP_OK == proxyPTZ.GetConfiguration(tptz__GetConfiguration, *tptz__GetConfigurationResponse)){
        ...
        //printf("Default PTZ Timeout: %ds\n", (int16_t)tptz__GetConfigurationResponse->PTZConfiguration->DefaultPTZTimeout / 1000);
...
int ClientPTZ::panLeft(std::string profileToken, int nDegrees){
    float pan = -nDegrees*0.0055555556;
    return absoluteMove(profileToken, pan, 1, 0.0, 0.0, 0.0, 0.0);
}

int ClientPTZ::panRight(std::string profileToken, int nDegrees){
    float pan = nDegrees*0.0055555556;
    return absoluteMove(profileToken, pan, 1, 1.0, 1, 0.0, 0.0);
}

int ClientPTZ::tiltDown(std::string profileToken, int nDegrees){
    float tilt = -nDegrees*0.0055555556;
    return absoluteMove(profileToken, 0.0, 0.0, tilt, 1.0, 0.0, 0.0);
}

int ClientPTZ::tiltUp(std::string profileToken, int nDegrees){
    float tilt = nDegrees*0.0055555556;
    return absoluteMove(profileToken, 0.0, 0.0, tilt, 1.0, 0.0, 0.0);
}

int ClientPTZ::zoomIn(std::string profileToken){
    return absoluteMove(profileToken, 0.0, 0.0, 0.0, 0.0, 0.05, 1.0);
}

int ClientPTZ::zoomOut(std::string profileToken){
    return absoluteMove(profileToken, 0.0, 0.0, 0.0, 0.0, -0.05, 1.0);
}
